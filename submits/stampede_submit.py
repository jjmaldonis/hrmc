...
def submit_job(paramfile, prev_jobid, s, ds, modelfile):
    """ Pass None to prev_jobid if using param_file.in as paramfile """
    
    # Call sed to change starting step number to s
    args = "sed -i '5s/.*/{0} {1}   !startingstep, how often we decrement temp and max move/' {2}".format(s,ds,paramfile)
...
def main():
    ...
    ds = 200000  # how often we decrement temp and max move
    ...
    for s in range(ss, ts, js):
        if(s > ss):
            ...
            jobid = submit_job(dst,jobid,s,ds,'model_final_{0}.txt'.format(jobid))
        else:
            # On the first run, just submit the starting parameter file
            dst = starting_param_filename
            jobid = submit_job(dst,None,s,ds,None)
